...
$package = Join-Path $stagingRoot 'LabPhotoTools-0.1.17-preview'
...
foreach($name in @('Install.ps1','Uninstall.ps1','Common.ps1','Bootstrap-Python.ps1','Runtime-Downloads.ps1','Setup-Entry.ps1','Get-Status.ps1')) {
...
$zip = Join-Path $dist 'LabPhotoTools-0.1.17-preview.zip'
...
$sourcePackage = Join-Path $stagingRoot 'LabPhotoTools-0.1.17-source'
New-Item -ItemType Directory -Path $sourcePackage | Out-Null
foreach($name in @('src','tests','scripts','installer','engine','README.md','INSTALL_KO.txt','VALIDATION.md','Install.cmd','Update.cmd','Uninstall.cmd')) {
    Copy-Item -LiteralPath (Join-Path $root $name) -Destination $sourcePackage -Recurse
}
Compress-Archive -LiteralPath $sourcePackage -DestinationPath (Join-Path $dist 'LabPhotoTools-0.1.17-source.zip') -Force
